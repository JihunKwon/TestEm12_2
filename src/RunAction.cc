...
:G4UserRunAction(),
 fDetector(det),fPhysics(phys),fPrimary(kin),fRun(0),fHistoManager(0),
	fShellEdep(new G4double[MaxShell])
{
...
void RunAction::EndOfRunAction(const G4Run*)
{
  ...
  // Output the csv file
retry1:
  FILE*fp = fopen("RadialEdep.csv", "wt");
  if (fp == NULL)
  {
	  G4cout << "Failed to open RadialEdep.csv!" << G4endl;
	  getchar();
	  goto retry1;
  }
  else
  {
	  fprintf(fp, "Shell number, Edep [MeV]\n");
...
void::RunAction::SaveRadialEdep(G4double r, G4double edep, G4int ShellNb)
{
	if (edep < 0.0)
		return;

	int i = ShellNb - 1;
	if (i < NUMBER_OF_SHELLS)
	{
		pRadialEdep[i] += edep;
	}
}
